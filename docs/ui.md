...
login. The DoH hostname does **not** serve this UI when
`DNS:DOH:IsolateHost` is true (default) — those requests 404 except
`/dns-query`. Use a different `Host` (or HTTP 8080), or set
`DNS__DOH__IsolateHost=false`.
